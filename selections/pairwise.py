...
class MyOptimizedCauchyPairwiseComparisonSelection(Selection[List[S], S]):
    def __init__(self):
        super(MyOptimizedCauchyPairwiseComparisonSelection, self).__init__()
        self.flag_list = True
    ...
    def execute(self, front: List[S], number=None) -> S:
        ...
        if len(front) == 1:
            ...
            for i, tupl in enumerate(combinations(range(len(front)), 2)):
                x, y = tupl
                val = self.evaluate(front[x], front[y], delta[i])
                matrix[x][y] = val
                matrix[y][x] = -val
            sums = [sum(row) for row in matrix]
            # save(sols, sums, number, self.get_name())
    ...
    def get_name(self) -> str:
        return "MyOptimizedCauchyPairwiseComparisonSelection"
